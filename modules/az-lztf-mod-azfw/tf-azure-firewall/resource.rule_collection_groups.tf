...
resource "azurerm_firewall_policy_rule_collection_group" "network_fqdns" {
  count = length(var.fw_network_fqdns_rules)

  depends_on = [
    azurerm_firewall.main,
  ]

  name               = "${var.policy_name}-rcg-netfqdns-${count.index + 1}"
  firewall_policy_id = azurerm_firewall_policy.main.id
  priority           = (250 + count.index)

  network_rule_collection {
    name     = var.fw_network_fqdns_rules[count.index].name
    priority = var.fw_network_fqdns_rules[count.index].priority
    action   = var.fw_network_fqdns_rules[count.index].action

    dynamic "rule" {
      for_each = [for rule in var.fw_network_fqdns_rules[count.index].rules : {
        name              = rule.name
        source_addresses  = rule.source_addresses
        destination_ports = rule.destination_ports
        destination_fqdns = rule.destination_fqdns
