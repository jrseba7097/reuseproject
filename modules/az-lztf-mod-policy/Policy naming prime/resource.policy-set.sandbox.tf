resource "azurerm_policy_set_definition" "sandbox" {
  name                  = "bramblesNamingSandbox"
  policy_type           = "Custom"
  display_name          = "Brambles Naming policies - Experimental"
...
VALUE
  ...
  policy_definition_reference {
    policy_definition_id = azurerm_policy_definition.vnet_sandbox_naming.id
    parameter_values     = <<VALUE
{
